...
from helper_functions import go_home, open_cabinet, open_microwave, pick_cube
...
ALLOWED_ACTIONS = {"open_microwave", "open_cabinet", "pick_cube", "go_home"}
...
def _sync_teleop_targets_from_robot():
    """Prevent jumps when switching back from auto mode."""
...
def _go_default_home(control_steps: int = 220):
    """
    Move robot to the startup default joint pose (jointStartPositions) and open gripper.
    """
    arm_home = jointStartPositions[:9]
    for _ in range(max(1, int(control_steps))):
        p.setJointMotorControlArray(
            panda.panda,
            range(9),
            p.POSITION_CONTROL,
            targetPositions=arm_home,
            positionGains=[0.08] * 9,
        )
        panda.open_gripper()
        p.stepSimulation()
...
def _plan_task(task_text: str):
    ...
    # Partial support for complex task: place cube in microwave.
    # For now we do supported helpers then handoff to teleop.
    if ("place" in text or "put" in text) and "cube" in text and "microwave" in text:
        cube_name = _parse_cube_name(text)
        steps = [{"action": "open_microwave"}, {"action": "pick_cube", "cube": cube_name}]
        return {
            "steps": steps,
            "handoff_message": "completed supported steps; type 'teleop' if you want manual control for final placement.",
            "message": f"planned partial support: open microwave + pick {cube_name}",
        }

    steps = []
    if "open microwave" in text:
        steps.append({"action": "open_microwave"})
    if "open cabinet" in text:
        steps.append({"action": "open_cabinet"})
    if "pick" in text and "cube" in text and "microwave" not in text:
    ...
    if not steps:
        return {"steps": [], "handoff_message": "", "message": "unsupported task. use teleop."}
